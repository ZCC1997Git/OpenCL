...
void Convolution(int imageInSizeX,
                 int imageInSizeY,
                 const T* imageIn,
                 const T* filter,
                 T* imageOut) {
...
#pragma unroll
                for (int fx = 0; fx < filterSize; fx++) {
                    ...
                    auto ImageItem =
                        imageIn[(fy + y) * imageInSizeX + (fx + x)];
...
void Convolution_unroll(int imageInSizeX,
                        int imageInSizeY,
                        T* imageIn,
                        T* filter,
                        T* imageOut) {
...
template <size_t filterSize, int BX, int BY, class T>
void Convolution_unroll_ref(int imageInSizeX,
                            int imageInSizeY,
                            T* imageIn,
                            T* filter,
                            T* imageOut) {
...
#pragma unroll
                for (int j = 0; j < BX; j++) {
                    imageOut[(y + i) * imageOutSizeX + (x + j)] =
                        sum[i * BX + j];
                }
            }
        }
...
template <size_t filterSize, int BX, int BY, class T>
void Convolution_unroll_simd_avx(int imageInSizeX,
                                 int imageInSizeY,
                                 T* imageIn,
                                 T* filter,
                                 T* imageOut) {
    ...
    for (int y = 0; y < imageOutSizeY; y += BY) {
        for (int x = 0; x < imageOutSizeX; x += BX * 8) {
            ...
            for (int fy = 0; fy < filterSize; fy++) {
                for (int fx = 0; fx < filterSize; fx++) {
                    __m256i filterItem =
                        _mm256_set1_epi32(filter[fy * filterSize + fx]);
                    auto lam = [&](int i, int j) [[gnu::always_inline]] {
                        ...
                        sum[j][i] = _mm256_add_epi32(
                            sum[j][i],
                            _mm256_mullo_epi32(filterItem, ImageItem));
                    };
                    loop_unrool<0, BX, BY>(lam);
                }
            }
            auto lam = [&](int i, int j) [[gnu::always_inline]] {
                _mm256_storeu_si256(
                    (__m256i*)(imageOut + (y + j) * imageOutSizeX +
                               (x + i * 8)),
                    sum[j][i]);
            };
            loop_unrool<0, BX, BY>(lam);
...
template <size_t filterSize, int BX, int BY, class T>
void Convolution_unroll_simd_avx512(int imageInSizeX,
                                    int imageInSizeY,
                                    T* imageIn,
                                    T* filter,
                                    T* imageOut) {
    ...
    for (int y = 0; y < imageOutSizeY; y += BY) {
        for (int x = 0; x < imageOutSizeX; x += BX * 16) {
            ...
            for (int fy = 0; fy < filterSize; fy++) {
                for (int fx = 0; fx < filterSize; fx++) {
                    __m512i filterItem =
                        _mm512_set1_epi32(filter[fy * filterSize + fx]);
                    auto lam = [&](int i, int j) [[gnu::always_inline]] {
                        ...
                        sum[j][i] = _mm512_add_epi32(
                            sum[j][i],
                            _mm512_mullo_epi32(filterItem, ImageItem));
                    };
                    loop_unrool<0, BX, BY>(lam);
                }
            }
            auto lam = [&](int i, int j) [[gnu::always_inline]] {
                _mm512_storeu_si512(
                    (__m512i*)(imageOut + (y + j) * imageOutSizeX +
                               (x + i * 16)),
                    sum[j][i]);
            };
            loop_unrool<0, BX, BY>(lam);
