...
cl_context CreateContext(cl_platform_id Platform,
                         std::vector<cl_device_id> device, cl_uint num_device);
cl_context CreateContext(cl_platform_id Platform, cl_device_id device);

std::vector<cl_command_queue>
CreateCommandQueue(cl_context context, std::vector<cl_device_id> &device);
cl_command_queue CreateCommandQueue(cl_context context, cl_device_id device);

cl_program CreateProgram(cl_context context, cl_device_id device,
                         std::string KernelSource);
cl_int BuildProgram(cl_program program, cl_uint num_device,
                    std::vector<cl_device_id> device, std::string options = "");

cl_kernel CreateKernel(cl_program program, std::string kernel_name);

template <class Lam, class Args, size_t... Is>
constexpr void Detail_Loop(Lam lam, cl_kernel kernel, Args &&ArgTuple,
...
template <class... Args> void SetKernelArg(cl_kernel Kernel, Args &&...args) {
...
template <class... BUF> void ReleaseBuffer(BUF &&...buf) {
  (clReleaseMemObject(buf), ...);
}

template <class... Args> void ReleaseCPUBuf(Args &&...args) {
  (delete[] args, ...);
}

template <class... KERNEL>
requires(std::is_same_v<KERNEL, cl_kernel> &&...) void ReleaseSource(
